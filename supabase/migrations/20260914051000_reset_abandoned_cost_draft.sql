-- El proyecto tiene un primer intento de costos que nunca llegó al código: una
-- tabla `product_costs` con promedio por moneda, columnas `unit_cost` en
-- `document_items` e `inventory_movements`, y dos funciones reescritas para
-- llenarlas. Nada de eso se usa desde la aplicación —el cliente jamás envía
-- `unitCost` en un movimiento— y las tres tablas están vacías. Se retira antes
-- de instalar la contabilidad definitiva, que separa el costo en sus propias
-- tablas y lo consolida en córdobas con el tipo de cambio de cada operación.
-- En un proyecto nuevo esta migración no encuentra nada que quitar y sólo
-- reafirma las dos funciones tal como las dejaron las migraciones anteriores.
