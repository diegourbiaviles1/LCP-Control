...
import { parseAmount, parseOpeningCosts } from './openingCostImport'
import type { ReportSource } from './model'

const base: Product = {
  id: 'p1', barcode: 'LCP-001', name: 'Perfume', brand: 'Marca', category: 'arabian',
  gender: 'unisex', size: 100, unit: 'ml', price: 900, currency: 'NIO', minimumStock: 2, active: true,
}
function item(overrides: Partial<Product>, quantities: InventoryItem['quantities']): InventoryItem {
  return { product: { ...base, ...overrides }, quantities }
}
function source(inventory: InventoryItem[], costed: string[] = []): ReportSource {
  return {
...
describe('lectura de una lista de costos iniciales', () => {
  ...
  it('resuelve códigos con tabulación, punto y coma o coma y omite el encabezado', () => {
    const value = source([
      ...
      item({ id: 'p3', barcode: 'LCP-003', manufacturerBarcode: '7501111' }, { store: 1, warehouse: 1 }),
    ])
    ...
    expect(rows.map((row) => [row.productId, row.unitCost, row.stock])).toEqual([
      ['p1', 420.5, 3], ['p2', 1100.25, 4], ['p3', 980, 2],
    ])
  })
