...
function savedDraft(kind: DocumentKind): DocumentDraft {
  return {
    id: 'saved-draft', kind, reference: 'Borrador anterior', customer: 'Cliente',
    customerId: null, phone: '', taxId: '', currency: 'NIO', tier: 'emprendedor',
    payment: 'pending', location: 'store', validUntil: '2099-01-01', notes: '',
    createdAt: '2026-09-13T12:00:00Z',
    lines: [{ productId: 'demo-0001', name: 'Aurora Norte Cedro 01', barcode: 'DEMO-0001',
      size: '3.4 oz', quantity: 2,
      prices: { emprendedor: { NIO: 1000, USD: 25 }, vip: { NIO: 950, USD: 24 }, premium: { NIO: 900, USD: 22 } },
    }],
  }
}

it.each(['invoice', 'proforma'] as const)('actualiza los precios al reabrir un borrador de %s y avisa antes de emitir', async (kind) => {
  draftStore.items = [savedDraft(kind)]
  const user = await prepare(false, kind)
  await user.click(document.querySelector('.saved-drafts button') as HTMLButtonElement)
  expect(document.querySelector('.invoice-total')).toHaveTextContent('1,830.00')
  expect(screen.getByText(/Borrador abierto con los precios actuales/)).toBeInTheDocument()
  await user.selectOptions(screen.getByLabelText('Lista de precios'), 'premium')
  expect(document.querySelector('.invoice-total')).toHaveTextContent('1,610.40')
...
it('impide emitir una proforma reabierta con un producto retirado del catálogo', async () => {
  ...
  await user.click(document.querySelector('.saved-drafts button') as HTMLButtonElement)
  expect(screen.getByLabelText(/^Cantidad de /)).toHaveAccessibleDescription(/ya no está disponible/)
